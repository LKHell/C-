...
void make_GL_QUADS(GLfloat a, GLfloat b, GLfloat c)
{
	glBegin(GL_QUADS);
	glColor3f(a, b, c);
	glVertex3f(1, 0.0, -1);      // P1
...
void display() {
   ...
   // orange vextex
	...
	/*----------door_Vertical--------------*/
	set_display();
	glTranslatef(3.7, 0.0, -1.18);
	glScalef(0.2, 2.4, 1.0);
	make_GL_QUADS_front(1.0, 1.0, 1.0);
	/*----------door_Horizontal-----------------*/
	set_display();
	glTranslatef(3.2, 2.0, -1.18);
	glScalef(1.6, 0.16, 1.0);
